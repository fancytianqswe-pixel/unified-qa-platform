...
- GET  /api/skills                 — list on-disk skills (SKILL.md discovery; dashboard BFF)
- GET  /api/skills/detail          — single skill (query ``id``, e.g. ``h0:my-skill``)
- POST /api/skills/register        — create a skill under ``~/.hermes/skills/``
...
def _sse_keepalive_seconds() -> float:
    """SSE 无内容增量时发送 `: keepalive` 的间隔；过大会被代理判空闲断连。"""
    raw = os.getenv("HERMES_SSE_KEEPALIVE_SECONDS", "15").strip()
    try:
        v = float(raw)
    except ValueError:
        return 15.0
...
def _run_sse_poll_seconds() -> float:
    """GET /v1/runs/{id}/events 轮询队列超时（秒）。"""
    raw = os.getenv("HERMES_RUN_SSE_POLL_SECONDS", "60").strip()
    try:
        v = float(raw)
    except ValueError:
        return 60.0
...
def _xingyan_prefer_mcp_tools() -> bool:
    """统一质检平台：对话优先走已注册 MCP（如 MinerU、datasource），再回退到 skill_view 技能。"""
    raw = os.getenv("HERMES_XINGYAN_PREFER_MCP_TOOLS", "1").strip().lower()
    return raw not in ("0", "false", "off", "no")


# 注入为 ephemeral_system 前缀（见 _create_agent），不写入轨迹缓存；可用 HERMES_XINGYAN_PREFER_MCP_TOOLS=0 关闭。
_XINGYAN_MCP_FIRST_HINT = (
    "【统一质检平台 · 工具优先级】当任务可用「已注册的 MCP 工具」也可通过 skill_view 加载某技能说明完成时："
    "① 优先使用 MCP——文档/PDF/Office 解析优先 mineru_api_parse_and_wait（或 mcp_*_mineru_api_parse_and_wait）；"
    "禁止在 Hermes 内用 Python/终端手写 multipart 调 /file_parse；大文件勿用 mineru_api_sync_parse。"
    "同一轮最多 2 次 parse_and_wait；数据规则审核阶段 B 须逐条处理，禁止 5 条并行 MinerU。"
    "数据库探测优先 datasource_*。"
    "② 仅当无匹配 MCP、或 MCP 调用已失败，再用 skill_view 加载技能（如 ocr-and-documents）。"
    "③ 不要为走流程抢先 skill_view；能用 MCP 直达时不要再用技能内的 Python/终端路径重复实现。"
)


def _merge_xingyan_mcp_first_ephemeral(ephemeral: Optional[str]) -> Optional[str]:
    if not _xingyan_prefer_mcp_tools():
        return ephemeral
    if not ephemeral:
        return _XINGYAN_MCP_FIRST_HINT
    return f"{_XINGYAN_MCP_FIRST_HINT}\n\n{ephemeral}"


def _strip_trailing_openai_chat_path(base_url: str) -> str:
    """Hermes HTTP client appends ``/v1/chat/completions``; strip if caller pasted the full path."""
    u = base_url.strip().rstrip("/")
    while True:
        low = u.lower()
        if low.endswith("/v1/chat/completions"):
            u = u[: -len("/v1/chat/completions")].rstrip("/")
            continue
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _parse_user_llm_override(self, body: Any) -> Optional[Dict[str, str]]:
        """Optional per-request LLM endpoint from the Next BFF (xingyan_user_llm).

        Expects ``{ "base_url", "api_key", "model" }`` (``model_name`` accepted as alias).
        Opt-out: set ``HERMES_DISALLOW_USER_LLM_OVERRIDE=1`` on the gateway process.
        """
        raw = os.getenv("HERMES_DISALLOW_USER_LLM_OVERRIDE", "").strip().lower()
        if raw in ("1", "true", "yes"):
            return None
        if not isinstance(body, dict):
            return None
        u = body.get("xingyan_user_llm") or body.get("user_llm")
        if not isinstance(u, dict):
            return None
        base_url = _strip_trailing_openai_chat_path(str(u.get("base_url") or "").strip())
        api_key = str(u.get("api_key") or "").strip()
        model = str(u.get("model") or u.get("model_name") or "").strip()
    ...
    def _create_agent(
        ...
        When *llm_override* is provided (from ``xingyan_user_llm`` on the HTTP
        body), resolve_runtime_provider is used with explicit base_url/api_key
        and the model id is overridden for this agent instance only.
        """
        ...
        if llm_override:
            bu = (llm_override.get("base_url") or "").strip()
            ak = (llm_override.get("api_key") or "").strip()
            mn = (llm_override.get("model") or "").strip()
            if bu and ak and mn:
                try:
                    rt = resolve_runtime_provider(
                        requested=os.getenv("HERMES_INFERENCE_PROVIDER"),
                        explicit_base_url=bu,
                        explicit_api_key=ak,
                        target_model=mn,
                    )
                    runtime_kwargs = {
                    ...
                    logger.warning("[api_server] xingyan_user_llm resolve failed: %s", exc)
        ...
        agent = AIAgent(
            ...
            **runtime_kwargs,
            ...
            ephemeral_system_prompt=_merge_xingyan_mcp_first_ephemeral(ephemeral_system_prompt),
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        llm_override = self._parse_user_llm_override(body)
                    ...
                    if agent_task.done():
                        ...
                        while True:
                            try:
                                ...
                                break
                        break
                    if time.monotonic() - last_activity >= _sse_keepalive_seconds():
    ...
    # ------------------------------------------------------------------
    # Skills catalog API (platform dashboard / Next BFF)
    # ------------------------------------------------------------------

    async def _handle_skills_list(self, request: "web.Request") -> "web.Response":
        """GET /api/skills — Hermes SKILL.md discovery as JSON list."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        try:
            from gateway.platforms.skills_api import list_skills_json

            include_document = request.query.get("include_document", "").lower() in ("1", "true", "yes")
            items = list_skills_json(include_document=include_document)
            return web.json_response({"ok": True, "list": items})
        except Exception as e:
            logger.exception("[api_server] /api/skills list failed: %s", e)
            return web.json_response({"ok": False, "message": str(e), "list": []}, status=500)

    async def _handle_skills_detail(self, request: "web.Request") -> "web.Response":
        """GET /api/skills/detail?id= — one skill including full SKILL.md."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        skill_id = request.query.get("id", "").strip()
        if not skill_id:
            return web.json_response({"ok": False, "skill": None, "message": "id is required"}, status=400)
        try:
            from gateway.platforms.skills_api import get_skill_detail

            skill = get_skill_detail(skill_id)
            if not skill:
                return web.json_response({"ok": False, "skill": None, "message": "not found"}, status=404)
            return web.json_response({"ok": True, "skill": skill})
        except Exception as e:
            logger.exception("[api_server] /api/skills/detail failed: %s", e)
            return web.json_response({"ok": False, "skill": None, "message": str(e)}, status=500)

    async def _handle_skills_register(self, request: "web.Request") -> "web.Response":
        """POST /api/skills/register — create ~/.hermes/skills/<slug>/SKILL.md."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        try:
            ...
            return web.json_response({"ok": False, "message": "Invalid JSON"}, status=200)
        try:
            from gateway.platforms.skills_api import register_skill_from_center

            out = register_skill_from_center(body if isinstance(body, dict) else {})
            return web.json_response(out, status=200)
        except Exception as e:
            logger.exception("[api_server] /api/skills/register failed: %s", e)
            return web.json_response({"ok": False, "message": str(e)}, status=200)
    ...
    async def _handle_run_events(self, request: "web.Request") -> "web.StreamResponse":
        ...
        try:
            while True:
                try:
                    event = await asyncio.wait_for(q.get(), timeout=_run_sse_poll_seconds())
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Skills catalog (dashboard BFF)
            self._app.router.add_get("/api/skills", self._handle_skills_list)
            self._app.router.add_get("/api/skills/detail", self._handle_skills_detail)
            self._app.router.add_post("/api/skills/register", self._handle_skills_register)
