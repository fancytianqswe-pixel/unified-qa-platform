...
server.registerTool("datasource_test_connection", {
    description: "数据库连通性探测；等价于前端「草稿卡 / 数据中心」调用的 POST /api/datasource/test。八项齐备后优先调用本工具确认可连，再在对话末尾输出 hermes-datasource 或兼容 yaml 以生成草稿卡。",
    inputSchema: connSchema,
}, async (args) => {
    const r = await postJson("/api/datasource/test", asDbPayload(args));
    return {
        content: [{ type: "text", text: JSON.stringify({ httpStatus: r.status, ...(r.body || {}) }) }],
    };
});
server.registerTool("datasource_list_columns", {
    description: "读取表字段（当前实现为 MySQL SHOW COLUMNS）；等价 POST /api/datasource/columns。仅在 test 通过后、且 dbKind=mysql 时使用。",
    inputSchema: connSchema,
}, async (args) => {
...
server.registerTool("datasource_preview_sample", {
    description: "最多 5 行样例；等价 POST /api/datasource/preview。支持 selectedFields。仅在 MySQL 且连接可用时使用。",
...
server.registerTool("datasource_save_guidance", {
    description: "无副作用：返回如何将当前连接写入「数据中心」的说明。真正持久化只能由用户在浏览器点击草稿卡「保存到数据中心」或数据中心弹窗完成（localStorage）。模型在探活/预览成功后应调用本工具一次以提醒输出 hermes-datasource 块。",
