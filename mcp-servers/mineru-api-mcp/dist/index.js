...
function extractTaskStatus(body) {
...
const fileParseSchema = parseOptionsSchema.extend({
    filePath: z
        .string()
        .describe("本机可读文件的绝对路径；MCP 进程将读取该文件并以 multipart 上传（与 MinerU /file_parse、/tasks 一致）。"),
});
const taskIdSchema = z.object({
    taskId: z.string().min(8).describe("异步任务 ID（由 POST /tasks 返回的 task_id）"),
});
function defaultBackend(explicit) {
    if (explicit)
        return explicit;
...
function appendParseFormFields(form, fileName, buf, args) {
    ...
    if (args.serverUrl)
        form.append("server_url", args.serverUrl);
...
async function readLocalFileForUpload(filePath) {
...
server.registerTool("mineru_api_health", {
...
server.registerTool("mineru_api_sync_parse", {
    description: "POST /file_parse：同步上传（小文件/联调用）。大 PDF/Office 请用 mineru_api_parse_and_wait，避免长连接中断。",
    ...
    try {
        ...
        const out = serialized.length > MAX_SYNC_BODY_CHARS
...
async function submitParseTask(args) {
    let buf;
    let name;
    try {
        ({ buf, name } = await readLocalFileForUpload(args.filePath));
...
server.registerTool("mineru_api_submit_parse_task", {
    description: "POST /tasks：异步提交解析，立即返回 202 与 task_id；大文件请优先 mineru_api_parse_and_wait（内置轮询与重试）。",
    inputSchema: fileParseSchema,
}, async (args) => {
...
server.registerTool("mineru_api_parse_and_wait", {
    description: "推荐：POST /tasks 后由 MCP 进程内轮询状态直至 completed/failed；失败或网络异常时自动重试提交（次数见 MINERU_PARSE_MAX_RETRIES）。避免同步 /file_parse 长连接与 Hermes 内手写 Python 调 API。",
    inputSchema: fileParseSchema,
}, async (args) => {
...
server.registerTool("mineru_api_task_status", {
...
server.registerTool("mineru_api_task_result", {
