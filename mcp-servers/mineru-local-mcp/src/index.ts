...
function buildMineruArgs(args: z.infer<typeof parseSchema>): string[] {
  const a: string[] = ["-p", args.inputPath, "-o", args.outputDir];
  if (args.method) a.push("-m", args.method);
  if (args.backend) a.push("-b", args.backend);
  if (args.lang) a.push("-l", args.lang);
  if (args.serverUrl) a.push("-u", args.serverUrl);
  if (args.apiUrl) a.push("--api-url", args.apiUrl);
  if (args.startPageId !== undefined) a.push("-s", String(args.startPageId));
  if (args.endPageId !== undefined) a.push("-e", String(args.endPageId));
  if (args.formulaEnable !== undefined) a.push("-f", String(args.formulaEnable));
  if (args.tableEnable !== undefined) a.push("-t", String(args.tableEnable));
  if (args.extraCliArgs?.length) a.push(...args.extraCliArgs);
  return a;
}
...
server.registerTool(
  ...
  {
    description:
      "检查本机是否可调用 MinerU CLI：执行 `MINERU_EXECUTABLE --version`（或等价）。未安装时请按 MinerU 官方文档安装 `pip install mineru[...]` 并保证 PATH。",
  ...
  async () => {
    ...
    try {
      ...
      return { content: [{ type: "text" as const, text: JSON.stringify({ ok: true, command: cmd, output: text }) }] };
    } catch (e) {
...
server.registerTool(
  ...
  {
    description:
      "调用本机 `mineru` CLI 将文档解析到指定输出目录（-p/-o 及常用选项）。适合解析机与 MCP 同机、已安装完整 MinerU 与模型依赖的场景。大文件可能耗时数分钟，可设 MINERU_CLI_TIMEOUT_MS。",
